      ...
      try {
         if (groupSet.filter && !isAdmin && isBotAdmin && !m.fromMe) {
            ...
            if (body && (new RegExp('\\b' + toxic.join('\\b|\\b') + '\\b')).test(body.toLowerCase())) {
               ...
               if (warning > 2) return client.reply(m.chat, Func.texted('bold', `🚩 Warning : [ 3 / 3 ], good bye ~~`), m).then(() => {
               ...
               return client.reply(m.chat, `乂  *W A R N I N G* \n\nYou got warning : [ ${warning} / 3 ]\n\If you get 3 warnings you will be kicked automatically from the group.`, m).then(() => client.sendMessage(m.chat, {
                  delete: {
                     remoteJid: m.chat,
                     fromMe: isBotAdmin ? false : true,
                     id: m.key.id,
